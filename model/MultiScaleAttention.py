from model.transformer_decoder import SingleAttention
import torch
...
class Block(nn.Module):
    def __init__(self, emb_dim, num_heads, drop_path=0.):
        ...
        self.MHSA = SingleAttention(emb_dim=emb_dim, num_heads=num_heads)
        self.mlp = Mlp(in_features=emb_dim, hidden_features=emb_dim*4, out_features=emb_dim)
        self.proj = nn.Linear(emb_dim//12, emb_dim)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm1= nn.LayerNorm(emb_dim)
        self.norm2 = nn.LayerNorm(emb_dim)


    def forward(self, focal, rgb):
        rgb = self.proj(rgb)
        token_fea = torch.cat((rgb, focal), dim=1)
        token_fea = token_fea + self.drop_path(self.MHSA(token_fea))
        token_fea = self.norm1(token_fea)
        token_fea = token_fea + self.mlp(token_fea)
        token_fea = self.norm2(token_fea)
