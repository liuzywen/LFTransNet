...
@software: PyCharm
@file: transformer_decoder_noPos.py
...
class decoderBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.multiAttention = MSDeformAttn(dim, n_levels=4, n_heads=num_heads, n_points=4)
        self.multiAttention = MultiAttention(emb_dim=self.dim, num_heads=self.num_heads)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        self.query_embed = nn.Embedding(4, dim)
    ...
    def forward(self, x, y):
        ...
        x = x + self.drop_path(self.multiAttention(self.with_pos_embed(x, query_embed), reference_points, y, spatial_shapes,
                                               level_start_index))
...
if __name__ == '__main__':
    a = torch.randn(12, 1, 32)
    b = torch.randn(12, 64, 32)
    # token = nn.Parameter(torch.zeros(1, 1, 512)).expand(1, -1, -1)
    model = transfmrerDecoder(6, 4, 32)

    out = model(a, b)    # for x in out:
    print(out.shape)
    #     print(x.shape)
